...
fn convert_ceph<'a>(nums: impl IntoIterator<Item=&'a str>) -> Vec<usize> {
    ...
    for num in nums.into_iter() {
        ...
        for (res, digit) in result.iter_mut().zip(num.chars()) {
            let Some(this) = (digit as char).to_digit(10) else {
                continue;
            };
...
pub fn parse_ceph<'a>(i: &'a str) -> Input<'a> {
    ...
    let sizes: Vec<_> = separator.iter()
        .enumerate()
    ...
    let mut lines = i.lines()
        ...
        .map(split_ceph);
    let operations: Vec<_> = lines.by_ref().next().expect("could not get operations").into_iter().map(|chunk| {
        let op = chunk.trim();
        match op {
            "*" => Operation::Multiply,
            "+" => Operation::Sum,
...
pub fn part1(i: &str) -> usize {
    ...
    cols.iter().zip(ops).map(|(col, op)| {
        let col: Vec<_> = col.iter().map(|cell| cell.trim().parse::<usize>().expect("could not parse int")).collect();
        op.reduce(&col)
    }).sum()
}
...
pub fn part2(i: &str) -> usize {
    let (cols, ops) = parse_ceph(i);
    cols.iter().zip(ops).map(|(col, op)| {
        op.reduce(&convert_ceph(col.into_iter().cloned()))
    }).sum()
}
