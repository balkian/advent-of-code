...
pub fn parse(i: &str) -> Input {
    let (start, splitters) = i
        .lines()
        .enumerate()
        .flat_map(|(ix, line)| {
            line.chars()
                .enumerate()
                .map(move |(jx, cell)| ((ix as isize, jx as isize), cell))
        })
        .fold(
            (None, HashSet::new()),
            |(ref mut start, mut splitters), (pos, c)| {
                match c {
                    '^' => {
...
pub fn part1((start, splitters): &Input) -> usize {
    ...
    for level in start.0..=maxlevel {
        let split: Vec<_> = beams
            .extract_if(|x| splitters.contains(&(level, *x)))
            .collect();
        for x in split {
            collisions += 1;
            beams.insert(x - 1);
...
pub fn part2((start, splitters): &Input) -> usize {
    ...
    for level in start.0..=maxlevel {
        let split: Vec<_> = beams
            .extract_if(|x, _| splitters.contains(&(level, *x)))
            .collect();
        for (x, val) in split {
