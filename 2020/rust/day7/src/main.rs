...
lazy_static! {
    static ref OUTER: Regex = Regex::new(r"(?P<bag>[\w\s]+) bag[s]? contain (?P<others>.*).").unwrap();
    static ref INNER: Regex = Regex::new(r"(?P<number>\d+) (?P<bag>[\s\w]+) bag[s]?").unwrap();
}

fn calculate(tree: &HashMap<String,Vec<String>>){
    let mut opts: Vec<&String> = tree.get("shiny gold").expect("Shiny gold is not contained in any other bag").iter().collect();
    let mut idx = 0;
