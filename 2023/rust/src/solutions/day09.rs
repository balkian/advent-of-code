    ...
    input
        ...
        .map(|line| {
            line.split_whitespace()
                .map(|n| n.parse::<isize>().expect("could not convert number {n}"))
                .collect()
        })
        .collect()
}
...
fn derive(series: &TimeSeries) -> TimeSeries {
    series.windows(2).map(|v| v[1] - v[0]).collect()
}
...
fn reconstruct(series: &TimeSeries) -> isize {
    ...
    hist.iter()
        .fold(0, |acc, c| acc + c.last().expect("vec was empty"))
}
...
fn reconstruct_backwards(series: &TimeSeries) -> isize {
    ...
    hist.iter()
        .rev()
        .fold(0, |acc, c| c.first().expect("vec was empty") - acc)
}
