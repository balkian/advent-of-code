...
use nom::character::complete::{multispace1,anychar,digit1,alpha1,newline};
...
impl Condition {
    fn split(&self, gift: &MagicGift) -> (Vec<MagicGift>, Vec<MagicGift>) {
        ...
        debug_assert_eq!(passed.iter().chain(rejected.iter()).map(|mp| mp.count()).sum::<usize>(),
            gift.count());
        (passed, rejected)
...
impl<'a> Processor<'a> {
    ...
    fn value(&self) -> usize {
        self.accepted.iter().map(|gift| gift.attrs.iter().sum::<u16>() as usize).sum::<usize>()
    }
...
impl<'a> MagicProcessor<'a> {
    fn new(p: &'a Processor) -> Self {
        let mut mp = MagicProcessor { workflow: p.workflow.clone(), .. Default::default()};
        let mb = MagicGift{attrs: [1..=4000, 1..=4000, 1..=4000, 1..=4000]};
            ...
            for gift in gifts {
                ...
                for rule in self.workflow.get(&key).expect("unknown workflow {key}") {
                    ...
                    for gift in this_batch.iter() {
                        ...
                        assert_eq!(pass.iter().chain(reject.iter()).map(|mp| mp.count()).sum::<usize>(),
                            gift.count());
...
fn parse_gift(input: &str) -> IResult<&str, Gift> {
    let (rest, keyvalues) = delimited(tag("{"), separated_list1(tag(","), separated_pair(anychar, tag("="), digit1)), tag("}"))(input)?;
    let mut attrs = [0; 4];
...
fn parse_rule(input: &str) -> IResult<&str, Rule> {
    let (rest, (cond, output)) = tuple((opt(terminated(tuple((anychar, alt((tag("="), tag(">"), tag("<"))), digit1)), tag(":"))),
                            alpha1))(input)?;
    let condition = match cond {
        None => Condition::Always,
        Some((att, "<", val)) => Condition::Less(index_for_char(att), val.parse::<u16>().expect("could not parse {val}")),
        Some((att, ">", val)) => Condition::Greater(index_for_char(att), val.parse::<u16>().expect("could not parse {val}")),
        Some((att, "=", val)) => Condition::Equal(index_for_char(att), val.parse::<u16>().expect("could not parse {val}")),
...
pub fn parse(input: &str) -> Processor {
    ...
    Processor{workflow, buckets, accepted: vec![]} //, rejected: vec![]}
}
